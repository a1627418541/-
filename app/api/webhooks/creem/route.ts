  ...
  try {
    ...
    switch (eventType) {
      ...
      case "subscription.paid": {
        ...
        // 如果 metadata 中没有 userId，尝试用 customer.email 查找
        if (!userId && subscription.customer?.email) {
          ...
          if (user) {
            userId = user.id;
            console.log("Creem webhook resolved userId from email:", subscription.customer.email);
          }
        }

        if (!userId) {
          console.warn("Creem webhook missing userId in subscription metadata, customer:", subscription.customer);
          return NextResponse.json({ received: true });
        }
      ...
      case "subscription.canceled": {
        ...
        let userId = subscription.metadata?.userId;
        if (!userId && subscription.customer?.email) {
          const user = await prisma.user.findUnique({ where: { email: subscription.customer.email } });
